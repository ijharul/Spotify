...
// Predefined playlist info (replace/add your folders and songs here)
const playlists = {
    "lovesongs": {
        "title": "Love Songs",
        "description": "Romantic hits",
        "cover": "songs/lovesongs/cover.jpg",
        "songs": ["song1.mp3", "song2.mp3"]
    },
    "othersongs": {
        "title": "Other Songs",
        "description": "Random hits",
        "cover": "songs/othersongs/cover.jpg",
        "songs": ["songA.mp3", "songB.mp3"]
    }
};

// Load songs from a folder
async function getsongs(folder) {
    if (!playlists[folder]) return [];

    currFolder = folder;
    songs = playlists[folder].songs;
...
async function displayAlbums() {
    ...
    for (const folder in playlists) {
        const p = playlists[folder];
        cardContainer.innerHTML += `
            <div data-folder="${folder}" class="card">
                <div class="play">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" fill="none">
                ...
                <img src="${p.cover}" alt="song image">
                <h2>${p.title}</h2>
                <p>${p.description}</p>
            </div>`;
    }
...
// Main function
async function main() {
    await getsongs("othersongs"); // default playlist
